...
export default function AddProductForm({ onSuccess, onCancel }: AddProductFormProps) {
  ...
  return (
  <div className="p-6 sm:p-8 lg:p-12 w-full max-w-[90%] lg:max-w-6xl mx-auto mt-10 bg-white rounded-2xl shadow-lg border border-gray-100">
    <Toaster position="top-right" />
    <h2 className="text-2xl sm:text-3xl font-bold mb-6 text-center text-blue-700">เพิ่มสินค้าใหม่ 🛍️</h2>

    <form onSubmit={handleSubmit} className="space-y-6">
      {/* STEP 1: Basic Info */}
      {step === 1 && (
        <div>
          <h3 className="font-semibold mb-2 text-lg sm:text-xl">ขั้นตอนที่ 1: ข้อมูลทั่วไป</h3>
      ...
      {step === 2 && (
        <div>
          <h3 className="font-semibold mb-2 text-lg sm:text-xl">ขั้นตอนที่ 2: รูปภาพสินค้า</h3>
          <input
            type="file"
            accept="image/*"
            multiple
