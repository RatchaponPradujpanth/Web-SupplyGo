        ...
        <div className="bg-white rounded-lg shadow-sm mb-4 overflow-hidden">
          <div className="flex border-b-2">
            <button
              ...
              className={`flex-1 px-8 py-4 text-center font-semibold transition-all flex items-center justify-center gap-2 ${
                orderType === 'normal'
                ...
                <div key={`group-${group.group_buying_id}-${idx}`} className="bg-white rounded-lg shadow-sm p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-500 flex items-center gap-1">
                        <Clock size={14} />
                        เริ่มเมื่อ: {group.orders?.[0]?.created_at ? new Date(group.orders[0].created_at).toLocaleDateString('th-TH') : 'ไม่ระบุ'}
                      </p>
                    ...
                    <div className="px-3 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-800 flex items-center gap-1">
                      {group.status === 'active' && <><div className="w-2 h-2 bg-green-500 rounded-full"></div> เปิดรับ</>}
                      {group.status === 'completed' && <><CheckCircle size={12} /> สำเร็จ</>}
                      {group.status === 'cancelled' && <><XCircle size={12} /> ยกเลิก</>}
                      {group.status === 'expired' && <><Clock size={12} /> หมดอายุ</>}
                      {!['active', 'completed', 'cancelled', 'expired'].includes(group.status) && group.status}
                    </div>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div className="bg-blue-50 rounded-lg p-4 flex items-center gap-2">
                      ...
                      <div>
                        <p className="text-sm text-gray-600">สมาชิกในกลุ่ม</p>
                  ...
                  <div className="bg-gray-50 px-6 py-4 border-b">
                    <div className="flex justify-between items-center">
                      ...
                      <div className={`px-3 py-1 rounded-full text-xs font-medium flex items-center gap-1.5 ${
                        ...
                        {statusTab === 'to_ship' && <><Package size={14} /> รอจัดส่ง</>}
                        {statusTab === 'to_receive' && <><Truck size={14} /> กำลังจัดส่ง</>}
                        {statusTab === 'completed' && <><CheckCircle size={14} /> สำเร็จแล้ว</>}
                      </div>
                    </div>
                  </div>
                  <div className="p-6">
